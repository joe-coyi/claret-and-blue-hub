...
function PollCard({ p, votes, myVotes, user, pending, onVote }: any) {
  const pollVotes = votes.filter((v:any) => v.poll_id === p.id);
  const myPollVotes = myVotes.filter((v:any) => v.poll_id === p.id);
    ...
    <Card className="stadium-card p-6 border-0">
        ...
        {(p.options as any[]).map(opt => {
          ...
          return (
            <button
              key={opt.id}
              type="button"
              disabled={hasVoted || !user || pending || closed}
              ...
              className={`w-full text-left relative overflow-hidden rounded-md border px-4 py-3 transition-colors disabled:cursor-not-allowed
                ${myPick ? "border-accent" : previewPick ? "border-accent" : "border-border"}
            ...
            >
              {hasVoted && (
                <div
                  ...
                  style={{
                    width: `${pct}%`,
                    background: myPick ? "var(--gradient-claret)" : "color-mix(in oklab, var(--accent) 25%, transparent)",
                  }}
                />
              )}
      ...
      <div className="mt-4 flex items-center justify-between gap-3">
        ...
        {!hasVoted && !closed && user && (
          <button
            disabled={pending || (isLineup ? picks.length !== maxSel : picks.length === 0)}
            onClick={submit}
